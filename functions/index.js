...
// ===== Webhook 簽名驗證 =====
// LINE 每次發 webhook 都會附 X-Line-Signature header
// = Base64( HMAC-SHA256( channelSecret, rawBody ) )
// 驗不過 → 401，防止偽造 webhook 攻擊
function verifyLineSignature(req) {
  const signature = req.get('X-Line-Signature');
  if (!signature) return false;
  const secret = process.env.LINE_CHANNEL_SECRET || '';
  if (!secret) {
    console.error('[webhook] LINE_CHANNEL_SECRET 未設定，跳過簽名驗證');
    return true; // 環境變數沒設時先放行，避免鎖死系統
  }
...
app.post('/webhook', express.json(), async (req, res) => {
  ...
  if (!verifyLineSignature(req)) {
    console.warn('[webhook] 簽名驗證失敗，拒絕請求');
